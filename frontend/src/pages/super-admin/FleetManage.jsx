  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      result = result.filter(f => 
        ...
        f.fleetCode?.includes(searchTerm) ||
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">車隊管理</h1>
          <p className="text-gray-600 mt-1">管理所有車隊，可進行停權/復權操作</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white rounded-xl shadow-sm p-4">
          <p className="text-sm text-gray-500">總車隊數</p>
      ...
      <div className="bg-white rounded-xl shadow-sm p-8 text-center">
        ...
        <p className="text-gray-500 mb-2">目前沒有車隊資料</p>
        <p className="text-sm text-gray-400">當車隊註冊並通過審核後，會在這裡顯示</p>
      </div>
    </div>
  )
}
